...
const audioFiles = {
    ...
    great: 'audios/great.mp3',
    fantastic: 'audios/fantastic.mp3',
    excellent: 'audios/excellent.mp3',
    unbelievable: 'audios/unbelievable.mp3',
...
function handleDragStart(e) {
    ...
    const targetCell = e.target.closest('.piece-cell');
    ...
    const pieceElement = e.currentTarget; // The .piece element
    draggedPiece = {
...
function handleDragEnter(e) {
    e.preventDefault();
    const targetCell = e.target.closest('.cell'); // Ensure we target the cell itself
...
function handleDragLeave(e) {
    ...
    const relatedTarget = e.relatedTarget ? e.relatedTarget.closest('.cell') : null;

    if (targetCell && draggedPiece) {
        // Check if the mouse is moving to another cell that will be part of the *same* highlight area.
        // This prevents flickering when moving between adjacent cells of the potential drop zone.
        let stillOverHighlightArea = false;
        if (relatedTarget && highlightedCells.includes(relatedTarget)) {
            stillOverHighlightArea = true;
        }

        // Only remove highlight if we are truly leaving the potential placement area
        if (!stillOverHighlightArea) {
            // Don't remove immediately, let the enter on the new cell handle it.
            // But if relatedTarget is null (leaving board) or not a cell, definitely clear.
            if (!relatedTarget || !relatedTarget.classList.contains('cell')) {
                 removeDragOverHighlight();
            }
        }
    } else if (!targetCell) {
        // If leaving something that isn't a cell, clear highlights
        removeDragOverHighlight();
    }
}
...
function handleDrop(e) {
    ...
    const targetCell = e.target.closest('.cell');
    console.log("Drop detected on cell:", targetCell ? `${targetCell.dataset.row}, ${targetCell.dataset.col}` : "invalid target");

    if (targetCell && draggedPiece && window.currentPlacementValid) {
         const targetRow = parseInt(targetCell.dataset.row);
         const targetCol = parseInt(targetCell.dataset.col);

         // Calculate the actual top-left position based on where drag started relative to the piece
         ...
         console.log("Drop failed: Invalid target, no dragged piece, or invalid placement.");
         // Dragged element opacity/class is reset in dragend
          removeDragOverHighlight(); // Clean up visual cues
          if (draggedPiece && draggedPiece.element) {
               draggedPiece.element.classList.remove('dragging'); // Ensure class is removed on fail
          }
          draggedPiece = null; // Clear piece info on failed drop
     }
}
...
// Add Drop Listeners (sin cambios mayores necesarios aquí)
function addDropListeners(cell) {
   // ... (código existente)
}

// Add Drag Listeners (sin cambios mayores necesarios aquí)
